...
async fn get_task_secrets(params: GetTaskSecretParams<'_>) -> Result<(), aws_sdk_ecs::Error> {
    ...
    let td_output = client
        .describe_task_definition()
        .task_definition(params.familiy)
        .send()
        .await?;
    ...
    let secrets: Secrets = container_definitions
        .first()
        .unwrap()
        .secrets()
        .unwrap()
...
async fn main() -> Result<(), Error> {
    ...
    let region = args.aws_region_name.unwrap_or_else(|| "us-east-1".into());
    ...
    if Path::new(SECRETS_FILE_NAME).exists() {
        remove_file(SECRETS_FILE_NAME)
            .unwrap_or_else(|_| panic!("Couldn't delete {}", SECRETS_FILE_NAME));
    }
